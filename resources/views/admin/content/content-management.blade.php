...
<body>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit-card-title" class="form-label">Title</label>
                        <input type="text" class="form-control" id="edit-card-title">
                    </div>
                    <div class="mb-3">
                        <label for="edit-card-description" class="form-label">Description</label>
                        <textarea class="form-control" id="edit-card-description" rows="4"></textarea>
                    </div>
                    <div class="mb-3">
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && Array.isArray(response.cards)) {
                            ...
                            toastr.error('Failed to load cards.');
                            console.error('Cards response:', response);
                        }
                    },
                    error: function(xhr) {
                    ...
                    const cardHtml = `
                        <div class="card-container" data-card-id="${card.id}">
                            <div class="d-flex justify-content-between align-items-center">
                                <input type="text" class="card-title-input" value="${card.title}" data-card-id="${card.id}">
                                <button class="btn btn-outline-primary btn-sm edit-card-btn" data-card-id="${card.id}">
            ...
            $(document).on('change', '.card-title-input, .card-description-input', function() {
                const cardId = $(this).data('card-id');
                const card = cards.find(c => c.id === cardId);
                if (!card) return;

                const data = {};
                if ($(this).hasClass('card-title-input')) {
                    data.title = $(this).val();
                } else {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            card.description = response.card.description;
                            toastr.success('Card updated.');
                        } else {
                            toastr.error('Failed to update card.');
                        }
                    },
                    error: function(xhr) {
            ...
            // Update Button Label
            $(document).on('change', '.button-label-input', function() {
                const cardId = $(this).data('card-id');
                ...
                const button = card.buttons.find(btn => btn.action === action);
                if (!button) return;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            renderCards();
                            toastr.success('Button label updated.');
                        } else {
                            toastr.error('Failed to update button label.');
                        }
                    },
                    error: function(xhr) {
            ...
            $(document).on('click', '.edit-card-btn', function() {
                ...
                $('#edit-card-title').val(card.title);
                $('#edit-card-description').val(card.description || '');

                const $buttonsContainer = $('#edit-card-buttons');
            ...
            $(document).on('click', '.delete-button-btn', function() {
            ...
            $('#save-card-btn').click(function() {
                ...
                card.title = $('#edit-card-title').val();
                card.description = $('#edit-card-description').val();
                card.buttons = $('#edit-card-buttons .mb-2').map(function() {
                    return {
                        label: $(this).find('.button-label-input').val(),
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && response.card) {
                            ...
                            console.error('Add card response:', response);
                        }
                    },
                    error: function(xhr) {
            ...
            function loadElements(stepId, $container) {
                if (!formBuilderId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && Array.isArray(response.elements)) {
                            renderElements(stepId, response.elements, $container);
                        } else {
            ...
            // Render Elements
            function renderElements(stepId, elements, $container) {
                $container = $container || $(`.elements-container[data-step-id="${stepId}"]`);
                $container.empty();
